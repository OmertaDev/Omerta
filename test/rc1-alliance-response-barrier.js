...
const membership = process.argv.includes('--membership');
...
const configuration = { scenario: 'scoped-original-http-response-completion-control',
  originalHook: membership ? 'projection-events membership after canonical Family formation' : 'activity-wire after canonical crime',
  runnerSha256: sha256(runner), terminalHookSha256: sha256(blocks[0]), database: database.descriptor,
  control: 'Hold the exact original response-hook SELECT before native dispatch. The canonical mutation HTTP response must arrive while terminal lifecycle completion remains pending; release and await the real SELECT, then terminal hook and full invariants. Membership control permits the first pre-handler occurrence and holds exactly the second, post-response occurrence.',
  fixtures: membership ? 'One ordinary entrant receives only initialization level-75 respect; canonical check-in funds canonical formation. No cash/membership/deadline fixture.' : 'One unmodified ordinary entrant; no gameplay fixtures.',
  exclusions: ['No original worker schedule or long-horizon claim', 'No concurrent per-commit observation claim', 'No runtime changes'] };
...
Object.assign(process.env, { DATABASE_URL: database.url, RATE_LIMIT: 'off', INVITE_MODE: 'off', SOCIAL_VERIFY_MODE: 'off', POPULATION_OFF: 'on',
  WORLD_GRAPH_KERNEL: 'on', CORE_PROGRESSION: 'on',
...
try {
  ...
  if (membership) {
    ...
    const before = (await app.pool.query('SELECT id,respect,cash,bank,ammo FROM characters WHERE id=$1', [entry.id])).rows[0];
    const respect = PACING.LEVEL_DIVISOR * 74 ** 2;
    await app.pool.query('UPDATE characters SET respect=$2 WHERE id=$1', [entry.id, respect]);
    await proof.record({ kind: 'declared-initialization-only-fixture', before, after: { respect }, noOtherFixtureWrites: true });
    await request(actor, 'POST', '/v1/checkin', {}, 'formation-checkin');
  }
  ...
  if (membership) assert(me.character.level >= 75 && !me.character.gang && me.character.cash >= rules.family.foundCost);
  const heldSql = membership ? 'SELECT gm.gang_id FROM characters c JOIN gang_members gm ON gm.character_id=c.id\n      WHERE c.account_id=$1 AND c.alive=true'
    : 'SELECT name FROM characters WHERE account_id=$1 AND alive';
  if (membership) assert.equal(sha256(heldSql), 'c4ab05947b51c82d1c0f8135cf45716800b5d16601c373e4556ff73a8ffab428', 'Not the retained failing native SQL');
  originalQuery = app.pool.query.bind(app.pool);
  let heldResolve, nativeFinished = false, lifecycleFinished = false, heldCount = 0, matchingCount = 0;
  ...
  app.pool.query = (sql, values) => {
    ...
    matchingCount++;
    if (membership && matchingCount === 1) { events.push('original-pre-handler-membership-select'); return originalQuery(sql, values); }
  ...
  const responsePromise = app.inject({ method: 'POST', url: membership ? '/v1/gangs' : '/v1/crimes/' + crime.id,
    payload: membership ? { name: 'Response Barrier Family', tag: 'RB' } : { approach: 'standard' },
    headers: { authorization: 'Bearer ' + actor.token, 'idempotency-key': 'held-original-hook-mutation', 'x-rc1-response-completion': completion } });
  await bounded(entered, 'Exact original response-hook SELECT');
  ...
  assert.equal(matchingCount, membership ? 2 : 1, 'Wrong original query occurrence held');
  if (membership) assert.equal(response.json().ok, true);
  ...
  result = { status: 'PASS_SCOPED', originalHook: configuration.originalHook, ordinaryEntrants: 1, gameplayFixtures: membership ? 1 : 0,
    fixtureWritesAfterControlBaseline: 0, matchingOriginalSelects: matchingCount, heldOriginalSelects: 1, heldSqlSha256: sha256(heldSql), ordinaryResponseBeforeRelease: true,
