...
assert.equal(sha256(block), '7a70a9c050b359e04a63c1669ff0fb778531fb7067f53119f85bc89548a9ada0', 'Runner witness block changed; review and rebind control');
...
const instantiate = new Function('env', `const {observeResources,createNpcCarAcquisitionCommitObserver,currentInvocation,at,allianceEnabled,
 snapshotWorldResources,diagnosticPool,reconcileWorldResources,worldResourceHash,proof,resourceSummary,resourceCost,
 resourceStream,carMeltWitnessSummary,carAcquisitionWitnessSummary,canonicalJson,sha256,assert}=env;
  ...
  try {
    const { commitObserver, getError } = instantiate({ observeResources: enabled,
      ...
      worldResourceHash: sha256, reconcileWorldResources(before, after, options) {
        ...
        return { checks: [], unsupported: options.carMeltProvenance || options.carAcquisitionProvenance ? [{ kind: 'declared-synthetic-unknown' }] : [], cars: { lineage: [] } };
      },
      ...
      resourceStream: stream, carMeltWitnessSummary: witnessSummary, carAcquisitionWitnessSummary: acquisitionSummary, canonicalJson, sha256, assert });
    ...
    await query(scope === 'car' ? 'DELETE FROM cars WHERE id=$1' : scope === 'acquisition' ? 'INSERT INTO cars VALUES ($1)' : 'SELECT $1', [carId]);
    if (failure) {
    ...
    return { digest: stream.digest('hex'), witnessSummary, acquisitionSummary, native, artifacts, records };
