...
if (process.argv.includes('--postgres')) {
  ...
  const comparison = { format: 1, source, seed, epoch: SERIAL_EPOCH, seams: SERIAL_SEAMS,
  ...
  for (const [table, column, mutation] of [['characters', 'cash', (v) => Number(v) + 1],
    ...
    const changed = structuredClone(state); const row = JSON.parse(changed.tables[table][0]);
    assert(Object.hasOwn(row, column)); row[column] = mutation(row[column]); changed.tables[table][0] = JSON.stringify(row);
