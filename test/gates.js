...
// values trade places, or let a third job inherit one, while still finding both numbers somewhere.
{
  ...
  const expectedNativeCommands = [
    'pgquery', 'pgcheck', 'test:db-migration:postgres', 'phase2:definitions:postgres', 'phase2:lots:postgres',
...
// two biggest, which reads exactly like a clean bill of health.
{
  ...
  const POSTURE = {
    ...
    'crafting.js:CRAFTING_PREREQUISITES': 'cache: immutable server prerequisite service keyed by authentic crafting context; each request rechecks durable world, social, mystery and inventory authority in its own database scope',
    'crafting.js:SERVER_PREVIEWS': 'cache: per-request server-created actor marker keeps caller-authored facts out of advisory catalogs; every mutation independently checks locked facts and persistent quotas',
    'world-prerequisites.js:TOKENS': 'cache: opaque actor and predicate facts bound to one active client and read/write scope; every process derives its own facts from locked authority or one database read snapshot',
    'world-prerequisites.js:PREPARED': 'cache: one local planning marker per active transaction prevents extending its already locked world and item predicate set; database locks and conditional mutations retain durable authority',
