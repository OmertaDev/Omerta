...
import { reconcileOrderExpiry } from '../tools/rc1-world-lifecycle-resources.js';
...
const admission = JSON.parse(admissionBytes), inventory = [], inputs = [];
for (const gate of ['rc1-turf-contention', 'rc1-turf-terminal', 'rc1-family-custody', 'rc1-family-monopoly', 'rc1-family-fragmented', 'rc1-law-policy', 'rc1-aggression-lifecycle', 'rc1-heir']) {
  ...
  const listed = manifest.artifacts.find(row => row.file === 'resource-summary.json'); assert(listed); assert.equal(sha256(summaryBytes), listed.sha256);
...
for (const file of ['restricted-resource-change-0003986.json', 'restricted-resource-change-0003987.json']) {
  ...
  const indexed = manifest.artifacts.find(row => row.file === file); assert(indexed); assert.equal(sha256(bytes), indexed.sha256);
  const input = JSON.parse(bytes), before = { format: 1, tables: {} }, after = { format: 1, tables: {} };
  for (const table of WORLD_RESOURCE_TABLES) {
    const change = input.restrictedChanges.tables.find(row => row.table === table);
...
const report = { status: 'PASS_HISTORICAL_CLASS_INVENTORY', observerSource: source, inputs, inventory, expiryCases,
...
console.log(JSON.stringify({ status: report.status, observerSource: source.revision, components: inventory.length,
  retainedSharedUnknowns: inventory.reduce((sum, row) => sum + row.retainedUnknownCount, 0), historicalExpiryCases: expiryCases.length }));
