...
const wrongLender = structuredClone(after); wrongLender.loans[0].lender_character = 'borrower';
assert.throws(() => reconcileLoanLifecycle(before, wrongLender, { label: 'offer' }), /Unreconciled/);
...
const expiredGrace = Date.parse('2026-09-21T13:00:00.001Z');
assert.equal(reconcileLoanLifecycle(before, after, { label: 'loan sweep', logicalAt: expiredGrace }).carMoves.length, 1);
assert.throws(() => reconcileLoanLifecycle(before, after, { label: 'loan sweep', logicalAt: expiredGrace - 1 }), /before original grace/);
after.cars[0].character_id = 'unrelated'; assert.throws(() => reconcileLoanLifecycle(before, after, { label: 'loan sweep', logicalAt: expiredGrace }), /owner\/custody/);
...
after.transactions.push({ ...receipt('wanted', 'bounty:wanted', '-25000'), counterparty: 'borrower' });
reconcileLoanLifecycle(before, after, { label: 'loan sweep' });
after.transactions[0].counterparty = 'unrelated'; assert.throws(() => reconcileLoanLifecycle(before, after, { label: 'loan sweep' }), /Unreconciled/);
after.transactions[0].counterparty = 'borrower';
