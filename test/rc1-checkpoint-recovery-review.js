...
function rows(population = 2) {
  ...
  return { accounts, account_persistent: accounts.map(row => ({ account_id: row.id })),
    characters: accounts.map((row, i) => ({ id: 'c' + i, account_id: row.id, alive: true, is_npc: false,
      ...
      loc: 'docks', gta_at: null, jail_until: null })), gangs: [{ id: 'g', npc_flag: true }], gang_members: [], cars: [],
    mystery_instances: [], coordination_instances: [], coordination_definitions: [], operation_escrow: [], world_recipe_usage: [],
    coordination_claims: [], coordination_claim_grants: [] };
}
...
assert.deepEqual(reviewed.joined.assertions.map(row => row.status), ['SATISFIED', 'SATISFIED', 'SATISFIED', 'UNKNOWN', 'UNKNOWN']);
...
assert(reviewed.details.some(row => row.scope === 'knowledge' && row.status === 'UNKNOWN' && row.reason.includes('exposed')));
for (const mutate of [
