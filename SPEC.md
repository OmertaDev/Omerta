...
| | |
|---|---|
| Backend modules | **214** files, **93369** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **176** files, **97411** lines (`test/`) — ratio 1.04 test:src |
| HTTP routes | **766** registrations (**766** unique) |
| Database tables | **338** (`schema.sql`, 6865 lines) |
...
| Design + audit docs | **514** markdown files, **140488** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
...
Roughly **207,000 lines** of backend code, tests, schema and top-level contracts.
