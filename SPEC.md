...
| | |
|---|---|
| Backend modules | **164** files, **58242** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **120** files, **52566** lines (`test/`) — ratio 0.90 test:src |
...
| Design + audit docs | **416** markdown files, **88845** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
| Ledger invariants | 18 named escrow/identity checks + per-currency conservation, **drift-0** |

Roughly **119,000 lines** of code, tests, schema and contracts.
