...
| | |
|---|---|
| Backend modules | **216** files, **93861** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **181** files, **99177** lines (`test/`) — ratio 1.06 test:src |
| HTTP routes | **771** registrations (**771** unique) |
| Database tables | **338** (`schema.sql`, 6880 lines) |
| Client | **12914** lines (`public/index.html`, single file, zero dependencies) |
| Ops dashboard + wiki | `public/admin.html`, `public/wiki.html` |
| Smart contracts | **32** top-level Solidity files, **9810** lines, **878** declared test functions in top-level Foundry suites, plus 35 in nested suites and 11 invariant functions across the test tree; the release gate re-measures the passing suite |
...
| Design + audit docs | **528** markdown files, **142330** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
...
Roughly **210,000 lines** of backend code, tests, schema and top-level contracts.
