...
| | |
|---|---|
| Backend modules | **166** files, **58867** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **122** files, **54144** lines (`test/`) — ratio 0.92 test:src |
| HTTP routes | **717** registrations |
| Database tables | **247** (`schema.sql`, 3770 lines) |
| Client | **11431** lines (`public/index.html`, single file, zero dependencies) |
...
| Design + audit docs | **418** markdown files, **89167** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
| Ledger invariants | 20 named escrow/identity checks + per-currency conservation, **drift-0** |

Roughly **120,000 lines** of code, tests, schema and contracts.
