...
| | |
|---|---|
| Backend modules | **257** files, **103402** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **238** files, **110619** lines (`test/`) — ratio 1.07 test:src |
| HTTP routes | **798** registrations (**798** unique) |
| Database tables | **361** (`schema.sql`, 7272 lines) |
| Client | **13217** lines (`public/index.html`, single file, zero dependencies) |
...
| Design + audit docs | **595** markdown files, **147535** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
