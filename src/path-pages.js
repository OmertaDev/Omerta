...
@media(max-width:480px){.public-nav{padding-inline:14px}.public-nav__brand{font-size:17px}.path-nav-links a{padding-inline:8px;font-size:9px}.quiz-card{padding:20px}.quiz-prompt{font-size:30px}.quiz-option{font-size:15px}.path-index__links{grid-template-columns:1fr}.effect-card{min-height:150px}.path-actions{display:grid}.path-button{width:100%}}
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.path-button,.quiz-progress__fill{transition:none}}
@media(forced-colors:active){.path-display mark{color:Canvas;background:Highlight}.quiz-card,.result-brief,.effect-card,.mastery-lane,.operations-panel,.result-footer,.path-index__links a{border-color:CanvasText}.path-button,.quiz-option{forced-color-adjust:auto}.result-stamp::before{box-shadow:none}}
</style>`;
...
function pathTrack(payload){
  return fetch('/v1/path-quiz',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(Object.assign({session:pathSession()},payload))}).then(function(response){if(!response.ok)throw new Error('telemetry');return response.json()})
}`;
}
