...
export async function dedicateLandmark(ch, districtId, amount, client, h) {
  ...
  const cur = (await client.query('SELECT amount, holder_name, holder_account FROM landmarks WHERE district_id=$1 FOR UPDATE', [districtId])).rows[0];
  ...
  // `took` is a DISPLACEMENT marker and a self-raise displaces nobody: raising your own flex with the
  // old shape read "<your dynasty>'s plaque comes down" — fluent and false (Codex round 3). Only a
  // holder who is genuinely somebody else comes down.
  return { ok: true, district: districtId, place, name, amount: amt, spent: amt,
    took: cur && cur.holder_account !== ch.account_id ? cur.holder_name : null };
}
