...
import { reconcileNpcMarketOrder } from './rc1-npc-market-order-journal.js';
...
import { reconcileMarketResources } from './rc1-world-market-resources.js';
...
function reconcilePressureCash(before, after, receipts, checks, unsupported, classifiedBanks = new Set()) {
  const priorPeople = indexed(rows(before, 'characters'), row => row.id, 'characters');
  const people = indexed(rows(after, 'characters'), row => row.id, 'characters');
  ...
  for (const [id, person] of people) {
    const prior = priorPeople.get(id); if (!prior || exactSum([person.bank, negate(prior.bank)]) === '0' || touched.has(id) || classifiedBanks.has(id)) continue;
...
export function reconcileWorldResources(before, after, { identity = null, includeRestrictedChanges = false, carMeltProvenance = null, carAcquisitionProvenance = null, npcFamilyProvenance = null, seasonElectionProvenance = null, npcBoatProvenance = null, duelSelection = null, npcMarketOrderProvenance = null, quiescentGroupEvidence = null } = {}) {
  ...
  const lifecycleCash = reconcileLifecycleCash(before, after, { identity, receipts });
  checks.push(...lifecycleCash.checks);
  const pressureCash = reconcilePressureCash(before, after, receipts, checks, unsupported, lifecycleCash.bankOwners);
  ...
  const npcMarketOrder = reconcileNpcMarketOrder(before, after, receipts, npcMarketOrderProvenance, identity);
  checks.push(...npcMarketOrder.checks);
  ...
  const remainingCargo = state => rows(state, 'character_cargo').filter(row => !retirement?.cargoKeys.has(tuple(row.character_id, row.good_id)) && !npcCargo.cargoKeys.has(tuple(row.character_id, row.good_id)) && !marketResources.cargoKeys.has(tuple(row.character_id, row.good_id)));
  ...
  const observedOnly = ['account_gear', 'exchange_pool', 'bounties', 'commission_proposals', 'favors',
  ...
  const otherMarket = state => rows(state, 'market_listings').filter(row => !retirement?.listingIds.has(row.id) && !orderResources.listingIds.has(row.id) && !marketResources.listingIds.has(row.id) && !orderExpiry.listingIds.has(row.id) && !npcMarketOrder.listingIds.has(row.id));
  ...
  const unmatchedMembers = state => rows(state, 'gang_members').filter(row => !membership.memberIds.has(row.character_id) && !npcRecruitment.memberIds.has(row.character_id) && !familyEntry.founderMembers.has(row.character_id) && !familyDissolution.dissolved.has(row.gang_id));
  ...
  return { format: 1, identity, beforeHash: worldResourceHash(before), afterHash: worldResourceHash(after), receipts,
    ...
    npcRetirement: { movements: retirement?.movements || [], scope: 'Source-bound original NPC retirement with no active loan/Family/boat or nonempty auxiliary cleanup: exact escrow returns, wallet sink, car identities and cargo destruction.' },
    ...
    marketResources: { movements: marketResources.movements, scope: 'Public goods purchase, goods posting/taking/return, order fill/claim/refund: exact owner/cargo/listing/receipt and shared-tax equations. Aggregate receipt multisets do not imply per-listing commit order. Quote and eligibility authority remains canonical.' },
