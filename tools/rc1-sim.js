...
export async function runNativeSimulation({ population, seed, replicate, rounds = 2, progress = () => {} }) {
  ...
  let baseline;
  const conservation = async (label) => {
    if (!baseline) return;
    try {
      const current = await runLedgerInvariants(f.pool, { alert: false });
      verifyLedgerChecks(baseline, current, population);
      metrics.conservationSweeps = (metrics.conservationSweeps || 0) + 1;
      metrics.conservationAssertions = (metrics.conservationAssertions || 0) + current.checks.length;
  ...
  const engine = {
    ...
    async execute(account, input, key) {
      try {
        ...
        await conservation(`command ${account}/${input.executionId}`);
        return response;
      } catch (error) {
        metrics.rejectedCommands[error.code || error.name] = (metrics.rejectedCommands[error.code || error.name] || 0) + 1;
        await conservation(`rejected command ${account}/${input.executionId}`);
        throw error;
      }
    },
  };
  const tick = async () => {
    ...
    await conservation('Director tick');
    return result;
  };
  ...
  try {
    ...
    baseline = await runLedgerInvariants(f.pool, { alert: false });
    ...
    for (let i = 0; i < socialActors.length; i += 2) {
      ...
      await conservation(`social formation ${leader.id}`);
    }
    ...
    for (const [index, actor] of roster.entries()) {
      ...
      if (['economic', 'hoarder'].includes(actor.archetype)) {
        ...
        await f.acquireMaterials(actor.id); await conservation(`materials ${actor.id}`); view = await engine.snapshot(actor.id);
      }
    ...
    return { population, seed, replicate, rounds, database: process.argv.includes('--postgres') ? 'postgresql' : 'pg-mem',
      ...
      invariantChecks, notMeasured: ['distinct commands contending for the same world object', 'sustained concurrent population load', 'long-term starvation',
        ...
        'per-internal-transition accounting inside compound fixture helpers (command boundaries are checked)'],
    };
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  ...
  const populations = (options.populations || '25,100,250,500,1000').split(',').map(Number);
