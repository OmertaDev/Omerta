...
const checkTransition = async (label) => {
  ...
  assert.equal(current.checks.length, Object.keys(transitionBaseline).length, 'A transition audit lost an invariant');
  const changed = current.checks.filter((check) => !(check.name in transitionBaseline)
    || !Number.isFinite(check.drift) || Math.abs(check.drift - transitionBaseline[check.name]) > 0.01);
  transitionChecks++;
