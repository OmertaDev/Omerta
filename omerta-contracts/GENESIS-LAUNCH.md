...
Use the read-only sampler rather than hand-calculating from an explorer snapshot:

```powershell
$env:CHAIN_RPC_URL = 'https://rpc.mainnet.chain.robinhood.com'
npm --prefix .. run genesis:cadence -- --duration-seconds 180 --interval-seconds 30 --rpc-class public
```
...
preflight, Safe simulation/approvals, and two independent decodes that all bind
`calldataDigests.launchKeccak256` from `genesis:config`:

```powershell
npm --prefix .. run genesis:release -- C:\secure-launch-evidence\genesis-release-review.json
```

Two operators must compare and sign the resulting `manifestSha256`. The tool never signs or sends.
...
The final machine-readable manifest is generated by `genesis:release`; its required evidence schema,
fail-closed checks, and explicit limits are in `GENESIS-RELEASE-MANIFEST.md`.
