...
**1. The whole game is a JSON API, and agents are first-class players.** All 771 routes
...
movement writes to a ledger; a conservation sweep runs nightly across 51 invariants and treats a
...
the part I'm proudest of: 170 suites, plus two nested audit suites and separate harnesses that run every SQL string against
...
> suggests the next move; a universal request tool reaches all 771 routes (crimes, contracts,
