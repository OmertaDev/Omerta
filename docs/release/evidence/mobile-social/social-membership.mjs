...
try {
  for (const width of [320, 390]) for (const scenario of scenarios.filter((entry) => entry.name === 'shipment-redistribution')) {
    ...
    try {
      ...
      result.membership.contributionCommands = result.commands.filter((entry) => entry.account === newcomer && entry.type === 'operation.contribute');
      assert(result.membership.contributionCommands.length > 0, 'The newly admitted player must actually contribute');
      const newcomerBoard = await engine.snapshot(newcomer);
      result.membership.newcomerConsequences = newcomerBoard.consequences.map(({ title, description }) => ({ title, description }));
      const sharedConsequence = result.membership.newcomerConsequences.find((entry) => entry.description.includes('redistributed'));
      assert(sharedConsequence, 'The newly admitted contributor receives the shared consequence');
      assert((await newPage.locator('#tab-world').innerText()).includes(sharedConsequence.description), 'Shared consequence actually rendered');
      await newPage.screenshot({ path: path.join(output, width + '-newcomer-shared-consequence.png'), fullPage: true });
