...
The backend entry paths are explicit. **pg-mem:** `withPhase2Read(pool, () => acquireWholeItemGateAndRunOwnedItemTransaction())` takes the outer gate and known pg-mem no-snapshot path, then one item client/BEGIN. **PostgreSQL mutation:** check out one client, BEGIN the item transaction, and perform immutable exact registry reads on that client; do not wrap the mutation in `withPhase2Read(pool, ...)`, which would start a read-only snapshot. Any use of `withPhase2Read(client, callback)` inside the item transaction participates in that existing snapshot and cannot add a transaction. **Public coherent reads:** `withPhase2Read(pool, callback)` owns the repeatable-read/read-only snapshot, then internal item read protection executes on that callback client. A registry read callback cannot recursively start an item transaction/snapshot on its same connection. Invariants compose one collection callback in the same order and send alerts afterward. No broad transaction-failure fallback to unsnapshotted pool queries is allowed. Initialize backend capability before any boot helper that chooses compensation or snapshot behavior.
