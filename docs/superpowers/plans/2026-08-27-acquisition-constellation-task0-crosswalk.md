...
After five deployments the factory enters exact `FINALIZING`. One atomic transaction checks permissible constructor state, topology, independent guards, Authority/Core zero state, forced-ETH accounting, and empty BudgetBook/Intent/Reconciliation state. It finalizes in fixed order `BudgetBook -> Reconciliation -> Intent -> Core -> Authority`, checks zero-length returns and every final flag, then marks the factory final. Any failure rolls back all child activations. Every finalizer and every factory phase mutator permanently reverts afterward.
...
Task 0 is green only when the literal census, collision universe, ownership manifest, mutation fixtures, and oracle artifact checks pass, while an explicit artifact-missing RED test proves the six final artifacts are absent. The default external verifier exits `42` while they are absent; `-ExpectTask0Red` converts only that exact absence into success. Task 1 must replace absence with exact six-artifact conformance. Linux CI later invokes PowerShell 7 with the same script and arguments.
