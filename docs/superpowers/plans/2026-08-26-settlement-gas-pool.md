...
- The runtime-hash/getter checks prove consistency, not semantic successor provenance, and do not survive Byzantine Safe semantics. The Safe and launch ceremony must independently reproduce and verify successor source, compiler version/settings, runtime hash, non-proxy/non-delegating semantics, immutable identity, current/pending ownership, paused state, and `acceptMigration` behavior before proposal.
