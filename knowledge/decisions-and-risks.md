...
| Chain launch | Code may exist and be devnet-proven while production remains dormant. External audit is non-negotiable. | `CHAIN-DEPLOY.md`, `CHAIN-AUDIT-PACKET.md`, `LAUNCH-READINESS.md` |
| Agent fairness | Agent accounts use agent keys, stricter cadence and no human anti-Sybil faucets. | `AGENTS.md`, auth/rate-limit/growth code |
...
- **Current GitHub work:** PR #126 changes the interface system. This knowledge snapshot includes the
  current checkout, so compare its recorded revision/branch with `main` before treating UI facts as
  deployed facts.
...
| Chain threat model and external-review scope | `CHAIN-AUDIT-PACKET.md`, `CHAIN-DEPLOY.md` |
