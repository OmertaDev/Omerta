...
function canvas(index,eyebrow,title,subtitle,body) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(title.join(' '))}">
    ...
    ${text('OMERTÀ  /  MARKET',M,1293,20,C.paper,{spacing:1.4})}
...
for(const [name,source] of artwork) {
  ...
  portable=portable.replace(/(<svg[^>]*>)/,`$1\n<title>${esc(name.replace(/^\d+-/, '').replaceAll('-',' '))}</title>\n<desc>OMERTÀ Market. Built and tested; not deployed. Original outlined vector infographic.</desc>`);
...
fs.writeFileSync(path.join(here,'infographics-manifest.json'),JSON.stringify({format:'Original outlined SVG + PNG',dimensions:{width:W,height:H},status:'BUILT & TESTED · NOT DEPLOYED',sourceOfTruth:'omerta-contracts/docs/market/DESIGN.md',artwork:manifest},null,2)+'\n');
