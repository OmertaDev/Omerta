...
2. **A scoped agent-led review of the CONTRACTS *and* the off-chain EIP-712 signer** under
   ...
   **In the batch — 38 contracts + 1 interface, every contract carrying tests:**
   ...
   The six liquidity-automation contracts and `src/interfaces/ILiquidityHealth.sol` are covered by
   the [2026-09-08 local predeployment review](omerta-contracts/audits/2026-09-08-liquidity-automation/report.md).
   Its focused contract, PostgreSQL, local-EVM and full backend gates passed; the review manifests
   identify the exact source and evidence. This local review grants no activation authority.
   ...
   | Genesis participation | `GenesisWalletCap` | 0.28 ETH cumulative commitment per wallet; only the controller-bound CCA may validate; caller equals recipient; rejected bids roll back and refunds never replenish allowance; no NFT or identity gate |
   ...
   | restricted liquidity automation | `FeeRevenueRouter`, `LiquidityBuybackExecutor`, `ProtocolLiquidityVault`, `GenesisLifecycleController`, `KeeperGasVault`, `BankBufferVault` | mint remains 100% DEV; fixed non-mint recipients; canonical delivery before accounting; bounded buybacks and gas/asset budgets; protected full-range position custody; actual genesis migration proof; optional exact-asset Bank funding; `ILiquidityHealth` gates new issuance without blocking old claims |
